...
bool weather_parse(const char *data, weather_t *weather)
{
    ...
    /* 在data中查找第一次出现字符串"\"text\":\""，其中\"是转义字符，表示双引号 */
    ...
    p += strlen("\"text\":\"");//计算字符串的长度，即字符串中字符的个数（不包括字符串结束符 '\0'）
    /* 查找下一个双引号 */
    char *q = strchr(p, '\"');//在字符串中查找第一次出现字符\的位置。   
    //                   \"：表示双引号 "。因为双引号在 C 语言中用于界定字符串的边界，所以如果要在字符串中包含双引号，就需要使用 \" 来表示。
    if (q == NULL)
    {
        return false;
    }
    ...
    weather->weather[len] = '\0';//数组中只有0~len-1的元素有效，第len个元素是'\0'
